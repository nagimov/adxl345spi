...
const int chipselectDefault = 0; // default chip select (integer)
...
void printUsage() {
    printf( "adxl345spi (version %s) \n"
            ...
            "  -c, --csel CSEL     set the chipselect (CS_EN0 or CS_EN1)\n"
            "                      (default: %i = CS_EN0) [integer]\n"
            "\n"
            ...
            "", codeVersion, timeDefault, freqMax, freqDefault, chipselectDefault);
}
...
int main(int argc, char *argv[]) {
    ...
    int chipselect = chipselectDefault; //TOE EDIT: default chipselect = 0
    ...
    for (i = 1; i < argc; i++) {  // skip argv[0] (program name)
        ...
        // TOe Edit: get here the chip select argument
        else if ((strcmp(argv[i], "-c") == 0) || (strcmp(argv[i], "--csel") == 0)) {
        if (i + 1 <= argc - 1) {
            i++;
            chipselect = atoi(argv[i]);
            if ((chipselect < 0) || (chipselect > 1)) {
                printf("Invalid chip select specified!\n\n");
                printUsage();
                return 1;
            }
        }
    ...
    h = spiOpen(chipselect, speedSPI, 3);       // TOe Edit: do not use static 0 anymore, instead use chipselect variable to make chip select selectable =)
    data[0] = BW_RATE;
    data[1] = 0x0F;
    writeBytes(h, data, 2);
    data[0] = DATA_FORMAT;
